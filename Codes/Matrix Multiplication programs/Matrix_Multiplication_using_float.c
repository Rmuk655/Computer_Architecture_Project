...
void multiply_float(int m, int n, int p, int matrix_1[m][n], int matrix_2[n][p], int product[m][p])
{
    ...
    for (int i = 0; i < m; i++)
    {
        ...
        {
            product_float[i][j] = 0.0f;
            for (int k = 0; k < n; k++)
            {
                product_float[i][j] = product_float[i][j] + (float)matrix_1[i][k] * (float)matrix_2[k][j];
            }
            product[i][j] = (int)product_float[i][j];
        }
    }
}
...
int main()
{
    ...
    for (int i = 0; i < no_of_trials; i++)
    {
        ...
        multiply_float(m, n, p, matrix_1, matrix_2, product);
